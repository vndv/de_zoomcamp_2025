...
@dlt.resource(
    name='players_archives',
    table_format="iceberg",
    file_format="parquet",
    write_disposition='append',
    # columns={'partition_date': {'partition': True}},
    )
...
def players_games(
    ...
    def _get_archive(url: str) -> List[TDataItem]:
        ...
        try:
            ...
            for game in games:
                timestamp = datetime.datetime.fromtimestamp(game['end_time'])
                game['partition_date'] = timestamp.strftime('%Y-%m-%d')
